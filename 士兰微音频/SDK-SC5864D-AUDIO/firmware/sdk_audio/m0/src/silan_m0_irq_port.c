...
int pic_port_request(int id, hdl_t hdl)
{
	...
	NVIC_ClearPendingIRQ((IRQn_Type)irq_p->irqid);
	NVIC_SetPriority((IRQn_Type)irq_p->irqid, irq_p->priority);
	NVIC_EnableIRQ((IRQn_Type)irq_p->irqid);
	return err;
}
...
void pic_port_free(int id)
{
	...
	irq_p = &irq[id];
	NVIC_ClearPendingIRQ((IRQn_Type)irq_p->irqid);
	NVIC_DisableIRQ((IRQn_Type)irq_p->irqid);
