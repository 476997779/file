...
/** @defgroup USBD_CUSTOMHID_Exported_Defines
  * @{
  */ 
#define USBD_SHID_REPORT_DESC_SIZE     81
...
/**
  * @}
  */ 
typedef struct _USBD_CHID_PROP
{
	int32_t (*SetReportOutput)(uint8_t report_id, uint8_t *report, uint16_t report_length);
	int32_t (*SetReportFeature)(uint8_t report_id, uint8_t *report, uint16_t report_length);
	int32_t (*GetReportInput)(uint8_t report_id, uint8_t *report, uint16_t report_length);
	int32_t (*GetReportFeature)(uint8_t report_id, uint8_t *report, uint16_t report_length);
}
USBD_Hid_cb_TypeDef;
...
int32_t USBD_SHID_GetSendStatus(void);
void USBD_SHID_SetCallback(USBD_Hid_cb_TypeDef *cb);
